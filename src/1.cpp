...
bool MS5837::init(TwoWire &wirePort)
{
    ...
    {
        ms5837_tx_data[0] = MS5837_PROM_READ + (i << 1);
        if (i2c.write(device_address, ms5837_tx_data, 1))
            ;
...
void MS5837::read()
{
    ...
    ms5837_tx_data[0] = MS5837_CONVERT_D1_8192;
    if (i2c.write(device_address, ms5837_tx_data, 1))
        ;
    ...
    if (i2c.read(device_address, ms5837_rx_data, 3))
        ;
    D1_pres =
        ms5837_rx_data[2] + (ms5837_rx_data[1] << 8) + (ms5837_rx_data[0] << 16);

    ms5837_tx_data[0] = MS5837_CONVERT_D2_8192;
    if (i2c.write(device_address, ms5837_tx_data, 1))
        ;
    ThisThread::sleep_for(20ms);
